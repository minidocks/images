...
RUN apk --update add \
    ...
    && wget -O /tmp/plugins.zip https://github.com/pematon/adminer-plugins/archive/v1.6.zip && unzip -q /tmp/plugins.zip -d /tmp && cp /tmp/adminer-plugins*/*.php /var/www/plugins/ && rm /var/www/plugins/AdminerLoginServers.php \

    && wget -O /tmp/theme.zip https://github.com/pematon/adminer-theme/archive/v1.7.zip && unzip -q /tmp/theme.zip -d /tmp && cp -r /tmp/adminer-theme*/lib/* /var/www/ \
